...
float CalibrateAccelerometer() {
  ...
  float norm_z = a.acceleration.z;
  float average = norm_x + norm_y + norm_z;
  for(int i = 0; i<10; i++) {
    mpu.getEvent(&a, &g, &temp);
    float norm_x = a.acceleration.x;
    float norm_y = a.acceleration.y;
    float norm_z = a.acceleration.z;
    float average = ((average*i) + norm_x + norm_y + norm_z)/(i+1);
  }
  return average;
}
...
bool ReadAccelerometer(float* input,
  ...
  const float norm_y = a.acceleration.y;
  const float norm_z = a.acceleration.z;
  ...
  if (pending_initial_data && begin_index >= 128) {
